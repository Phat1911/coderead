...
/** Props passed from the server page after Shiki highlighting runs at build time. */
...
/** Tailwind badge classes keyed by difficulty. Shared across challenge list and detail views. */
...
export default function ChallengeView({ challenge, prev, next, highlightedCode }: ChallengeViewProps) {
  ...
  const { loading: bookmarksLoading, toggleBookmark, isBookmarked } = useBookmarks()
  ...
  /** Reveals the explanation and records the view in user_progress with tiered points based on difficulty. */
  async function handleReveal() {
    ...
    // Non-quiz challenges have no right/wrong answer — is_correct stays false.
    const points = calculatePoints('learn', challenge.difficulty)

    await supabase.from('user_progress').upsert(
      { user_id: user.id, challenge_id: challenge.id, is_correct: false, points },
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-[#0a0a0a] transition-colors duration-200">
      ...
      <div className="max-w-3xl mx-auto py-10 px-6">
        ...
        {/* Quiz or plain reveal — quiz challenges show options first, everything else is gated behind answer */}
        {challenge.options && challenge.options.length > 0 && challenge.correctOptionIndex !== undefined ? (
          <QuizQuestion challenge={challenge} nextChallengeId={next} />
        ) : (
